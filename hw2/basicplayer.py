...
def shrey_evaluate(board):
   all_lengths = [-1,-1,2,3,-1,4]
   score = 0
   if 0:
       # If the game has been won, we know that it must have been
       # won or ended by the previous move.
       ...
       for idx, length in enumerate(all_lengths): 
	       ...
	       ## This helps in weighing all the chains.   	
	       score += cur_player_chain[length] * pow(10,idx)  
           if length in other_player_chain:
	       score -= other_player_chain[length] * pow(10,idx)
   '''  
   if sys.stdout.encoding and 'UTF' in sys.stdout.encoding:
       print unicode(board)
   else:
...
shrey_player = lambda board: minimax(board, depth=4, eval_fn=shrey_evaluate)
